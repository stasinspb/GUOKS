...
for roots, dirs, files in os.walk(dir_path):
    for file in files:
        ...
        if file.endswith('.xml'):
            ...
            ############  работа с xml для определения имени файла пдф по имени объекта ################

            xml = os.path.join(roots, file)
            tree = ET.ElementTree(file=xml)
            root = tree.getroot()

            if root[0].tag == "Construction":               ########### определяем здание или сооружение
                zd = False
            ...
            if zd is True:                                          # определяем имя файлов с поэтажками
                for my_file in my_files:
            ...
            try:
                for p in plans:                                           # добавляем файлы с поэтажками
                    ...
                    with open(os.path.join(roots, dirs[0], 'jpg-to-pdf.pdf'), 'wb') as f:
                        f.write(pdf)
                    reader_input = PdfReader(os.path.join(roots, dirs[0], 'jpg-to-pdf.pdf'))     # добавляем созданный пдф
                    for current_page in range(len(reader_input.pages)):
                        writer_output.addpage(reader_input.pages[current_page])
            except:
